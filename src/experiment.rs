...
>>>>>>> 80b93fffa1353934f3fb0059ab86bf4f540c525a
...
pub fn _alternate_screen () -> Result<()> {
    ...
    stdout.execute(terminal::Clear(terminal::ClearType::All))?.execute(terminal::EnterAlternateScreen)?;
...
pub fn no_library () -> Result<()> {
    ...
    for x in 0..12 {
        for y in 0..12 {
            p_field[(y * 12 + x) as usize] = 
            if x == 0 || x == 11 - 1 || y == 11 - 1 {
                9
            } else {
                0
            }
    ...
    for x in 0..12 {
        for y in 0..12 {
            stdout.write(
                &[" ABCDEFG=#".as_bytes()[p_field[(y * 12 + x) as usize] as usize]]
            )?;
        }
    }
